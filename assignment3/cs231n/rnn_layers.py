...
def rnn_step_backward(dnext_h, cache):
    ...
    dtanh = np.multiply(dnext_h, 1 - next_h ** 2)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h[:, 0, :] = h0
    for i in range(T):
        h[:, i+1, :], cache = rnn_step_forward(x[:, i, :], h[:, i, :], Wx, Wh, b)
    h = h[:, 1:, :]
...
def rnn_backward(dh, cache):
    ...
    for i in range(T-1, -1, -1):
        if i == 0:
            ...
            prev_h = h[:, i-1, :]
        cache = (x[:, i, :], prev_h, Wx, Wh, b, h[:, i, :])
        dnext_h = dh[:, i, :] + dnextnext_h # Gradient directly from loss + Gradient from next next h
        dx[:, i, :], dprev_h, dWx_now, dWh_now, db_now = rnn_step_backward(dnext_h, cache)
        dnextnext_h = dprev_h
        dWx += dWx_now
        dWh += dWh_now
        db += db_now
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dtanh_c = np.multiply(dnext_h, o)
    dnext_c += np.multiply(dtanh_c, 1 - np.tanh(next_c) ** 2)
    ...
    dai = np.multiply(di, np.multiply(i, 1 - i))
    daf = np.multiply(df, np.multiply(f, 1 - f))
    dao = np.multiply(do, np.multiply(o, 1 - o))
    dag = np.multiply(dg, 1 - g ** 2)
...
def lstm_backward(dh, cache):
    ...
    #############################################################################
    N, T, H = dh.shape
    x, prev_h, prev_c, Wx, Wh, a, i, f, o, g, next_c, next_h = cache[-1]
    D = x.shape[1]
    ...
    for i in range(T-1, -1, -1):
        # the reverse of the forward loop
        dnext_h = dh[:, i, :]
        dnext_c = dprev_c
        dnext_h = dnext_h + dprev_h
        dx[:, i, :], dprev_h, dprev_c, dWx_now, dWh_now, db_now = \
            lstm_step_backward(dnext_h, dnext_c, cache[i])
        dWx += dWx_now
        dWh += dWh_now
        db += db_now
    dh0 = dprev_h
